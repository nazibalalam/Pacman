...
############################ HELPER FUNCTIONS ##################################
            ...
            for yidx, line in enumerate(file):
                for xidx, char in enumerate(line):
                    if char == "1":
                        ...
                        pygame.draw.rect(self.background, self.BLACK, (xidx*self.cell_width, yidx*self.cell_height,
    ...
    def draw_grid(self):
        for x in range(self.WIDTH//self.cell_width):
            pygame.draw.line(self.background, self.GREY, (x*self.cell_width, 0),
                             (x*self.cell_width, HEIGHT))
        for x in range(self.HEIGHT/self.cell_height):
            pygame.draw.line(self.background, self.GREY, (0, x*self.cell_height),
...
########################### INTRO FUNCTIONS ####################################
    ...
    def start_draw(self):
        ...
        self.draw_text('PUSH SPACE BAR', self.screen, [
                       self.WIDTH//2, self.HEIGHT/2-50], self.START_TEXT_SIZE, (170, 132, 58), self.START_FONT, centered=True)
        self.draw_text('1 PLAYER ONLY', self.screen, [
                       self.WIDTH//2, self.HEIGHT/2+50], self.START_TEXT_SIZE, (44, 167, 198), self.START_FONT, centered=True)
        self.draw_text('HIGH SCORE', self.screen, [4, 0],
                       self.START_TEXT_SIZE, (255, 255, 255), self.START_FONT)
...
########################### PLAYING FUNCTIONS ##################################
    ...
    def playing_draw(self):
        ...
        self.draw_text('HIGH SCORE: 0', self.screen, [
                       self.WIDTH//2+60, 0], 18, self.WHITE, self.START_FONT)
        self.player.draw()
        for enemy in self.enemies:
            enemy.draw()
        pygame.display.update()
        ...
        for coin in self.coins:
            pygame.draw.circle(
                ...
                (
                    ...
                    int(coin.y*self.cell_height)+self.cell_height/2+self.TOP_BOTTOM_BUFFER//2),
                5
            )
...
########################### GAME OVER FUNCTIONS ################################
    ...
    def game_over_draw(self):
        ...
        self.draw_text("GAME OVER", self.screen, [
                       self.WIDTH//2, 100],  52, RED, "arial", centered=True)
        self.draw_text(again_text, self.screen, [
                       self.WIDTH//2, self.HEIGHT/2],  36, (190, 190, 190), "arial", centered=True)
        self.draw_text(quit_text, self.screen, [
                       self.WIDTH//2, self.HEIGHT/1.5],  36, (190, 190, 190), "arial", centered=True)
        pygame.display.update()
