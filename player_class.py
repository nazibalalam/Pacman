...
class Player(settings.Setting):
    ...
    def draw(self):
        pygame.draw.circle(self.app.screen, self.PLAYER_COLOUR, (int(self.pix_pos.x),
                                                                 int(self.pix_pos.y)), self.app.cell_width//2-2)

        # Drawing player lives
        for x in range(self.lives):
            pygame.draw.circle(
                self.app.screen, self.PLAYER_COLOUR, (30 + 20*x, self.HEIGHT - 15), 7)

        # Drawing the grid pos rect
        # pygame.draw.rect(self.app.screen, RED, (self.grid_pos[0]*self.app.cell_width+self.TOP_BOTTOM_BUFFER//2,
        #                                         self.grid_pos[1]*self.app.cell_height+self.TOP_BOTTOM_BUFFER//2, self.app.cell_width, self.app.cell_height), 1)

    def on_coin(self):
        if self.grid_pos in self.app.coins:
            if int(self.pix_pos.x+self.TOP_BOTTOM_BUFFER//2) % self.app.cell_width == 0:
                if self.direction == vec(1, 0) or self.direction == vec(-1, 0):
                    return True
            if int(self.pix_pos.y+self.TOP_BOTTOM_BUFFER//2) % self.app.cell_height == 0:
